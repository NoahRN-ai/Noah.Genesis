...
from vertexai.language_models import (  # For type hinting
    TextEmbeddingModel,
    TextEmbeddingsPrediction,
)
...
RAG_GCS_BUCKET_NAME = os.getenv(
    "RAG_GCS_BUCKET_NAME"
)  # Output from terraform/vector_search.tf
EMBEDDINGS_GCS_PREFIX = os.getenv(
    "EMBEDDINGS_GCS_PREFIX", "embeddings_input"
)  # Matches terraform var
CHUNK_MAP_GCS_OBJECT_NAME = os.getenv(
    "CHUNK_MAP_GCS_OBJECT_NAME", "metadata/id_to_chunk_details_map.json"
)  # Matches terraform var
...
def load_documents_from_path(path: Path) -> list[dict[str, Any]]:
    ...
    if not path.exists() or not path.is_dir():
        logger.error(
            f"Source documents directory not found or is not a directory: {path.resolve()}"
        )
        return []
        ...
        if file_path.is_file():
            try:
                ...
                if content.strip():
                    docs_content.append(
                        {
                            "source_document_name": file_path.name,
...
def chunk_documents(
    docs: list[dict[str, Any]], chunk_size: int, chunk_overlap: int
) -> list[dict[str, Any]]:
    ...
    for doc in docs:
        try:
                ...
                all_chunks.append(
                    {
                        ...
                        "source_document_name": chunk_doc.metadata[
            ...
            logger.info(
                f"Chunked {doc['source_document_name']} into {len(chunks)} chunks."
            )
        except Exception as e:
...
def prepare_vector_search_data(
    chunks_with_ids: list[dict[str, Any]], embeddings: list[list[float]]
) -> list[dict[str, Any]]:
                ...
                "restricts": [  # Example restricts for filtering - customize as needed
                    {
                        ...
                        "allow_list": [chunk_info["source_document_name"]],
                    },
                    {
                        "namespace": "chunk_index",
                        "allow_list": [str(chunk_info["chunk_index_in_doc"])],
                    },
                ],
...
def upload_to_gcs(
    ...
    try:
        ...
        logger.info(
            f"Successfully uploaded data to gs://{bucket_name}/{destination_blob_name}"
        )
    except Exception as e:
        logger.error(
            f"Failed to upload to GCS gs://{bucket_name}/{destination_blob_name}: {e}"
        )
        raise
...
def run_rag_pipeline():
    ...
    if not all([GCP_PROJECT_ID, GCP_REGION, RAG_GCS_BUCKET_NAME]):
        logger.error(
            "Missing critical environment variables: GCP_PROJECT_ID, GCP_REGION, RAG_GCS_BUCKET_NAME."
        )
        logger.error(
            "Please set these variables. They can be found in Terraform outputs or your project config."
        )
        return
    ...
    try:
        ...
        logger.info(
            f"Vertex AI initialized for RAG pipeline in project {GCP_PROJECT_ID}, region {GCP_REGION}."
        )
    except Exception as e:
    ...
    # 2. Text Chunking
    logger.info(
        f"Step 2: Chunking documents (Chunk Size: {CHUNK_SIZE}, Overlap: {CHUNK_OVERLAP})..."
    )
    ...
    logger.info(
        f"Step 3: Generating embeddings using Vertex AI model '{EMBEDDING_MODEL_NAME}'..."
    )
    ...
    if len(embedding_vectors) != len(chunked_data_with_ids):
        logger.error(
            f"Mismatch in number of embeddings ({len(embedding_vectors)}) vs chunks ({len(chunked_data_with_ids)}). Exiting."
        )
        return
    ...
    logger.info(
        "Step 4: Preparing data for Vertex AI Vector Search and ID-to-Chunk map..."
    )

    # Data for Vector Search embeddings batch import
    vector_search_jsonl_data = prepare_vector_search_data(
        chunked_data_with_ids, embedding_vectors
    )
    ...
    # 5. Upload to GCS
    logger.info(
        f"Step 5: Uploading generated files to GCS Bucket '{RAG_GCS_BUCKET_NAME}'..."
    )
    ...
    embeddings_jsonl_content = "\n".join(
        [json.dumps(item) for item in vector_search_jsonl_data]
    )
    embeddings_blob_name = (
        f"{EMBEDDINGS_GCS_PREFIX.strip('/')}/embeddings_batch_1.jsonl"
    )
    upload_to_gcs(
        RAG_GCS_BUCKET_NAME,
        embeddings_blob_name,
    ...
    upload_to_gcs(
        ...
        chunk_map_json_content,
        "application/json",
    )

    logger.info("--- RAG Document Processing Pipeline Finished Successfully ---")
    logger.info(
        f"Embeddings data uploaded to: gs://{RAG_GCS_BUCKET_NAME}/{embeddings_blob_name}"
    )
    logger.info(
        f"Chunk metadata map uploaded to: gs://{RAG_GCS_BUCKET_NAME}/{CHUNK_MAP_GCS_OBJECT_NAME}"
    )
    logger.info(
        "Next step: Ensure your Vertex AI Vector Search Index is configured to read from the embeddings GCS path if using BATCH_UPDATE at index creation, or use the gcloud CLI or SDK to update/rebuild the index with this data."
    )
