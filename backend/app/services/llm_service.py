    ...
    if lc_messages:
        for lc_msg in lc_messages:
            if isinstance(lc_msg, HumanMessage):
                ...
                model_parts = []
                if lc_msg.content and isinstance(lc_msg.content, str) and lc_msg.content.strip(): # Ensure content is not empty string for part
                ...
                response_data_dict = {"content": lc_msg.content} # Default if content is simple string
                try:
...
async def _generate_content_with_retry(
    ...
    tools: Optional[List[Any]] = None # Placeholder for vertexai.generative_models.Tool schema
