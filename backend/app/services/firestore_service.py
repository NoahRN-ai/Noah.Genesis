...
from google.cloud.firestore import (  # For typed queries if needed later
    Query,
)
...
from backend.app.models.firestore_models import (
    ...
    InteractionHistory,
    InteractionHistoryCreate,  # InteractionHistoryUpdate might not be used if immutable
    MedicationStatement,
...
async def create_user_profile(
    user_id: str, profile_data: UserProfileCreate
) -> UserProfile:
...
async def update_user_profile(
    user_id: str, profile_update_data: UserProfileUpdate
...
async def list_patient_data_logs_for_user(
    user_id: str, limit: int = 20, order_by: str = "timestamp", descending: bool = True
) -> list[PatientDataLog]:
    ...
    async for snapshot in query.stream():
        if (
            snapshot.exists
        ):  # Should always be true for query results unless collection is empty
            data = snapshot.to_dict()
            logs.append(PatientDataLog(log_id=snapshot.id, **data))
    return logs


async def update_patient_data_log(
    log_id: str, log_update_data: PatientDataLogUpdate
) -> Optional[PatientDataLog]:
...
async def create_interaction_history_entry(
...
async def get_interaction_history_entry(
    interaction_id: str,
...
async def list_interaction_history_for_session(
    ...
    query = db.collection(INTERACTION_HISTORY_COLLECTION).where(
    ...
    async for snapshot in query.stream():
        if snapshot.exists:
            ...
            history_entries.append(
                InteractionHistory(interaction_id=snapshot.id, **data)
            )
...
async def create_patient_profile(
    patient_id: str, profile_data: PatientProfileCreate
) -> PatientProfile:
...
async def update_patient_profile(
    patient_id: str, profile_update_data: PatientProfileUpdate
    ...
    if updated_snapshot.exists:
        return PatientProfile(
            patient_id=updated_snapshot.id, **updated_snapshot.to_dict()
        )
...
async def create_observation(
    patient_id: str, observation_data: ObservationCreate
) -> Observation:
    ...
    subcollection_ref = (
        db.collection(NOAH_MVP_PATIENTS_COLLECTION)
        .document(patient_id)
        .collection(NOAH_MVP_OBSERVATIONS_SUBCOLLECTION)
    )
    current_time = utcnow()
    ...
    _timestamp, doc_ref = await subcollection_ref.add(
        doc_data
    )  # Firestore auto-generates observation_id
...
async def get_observation(
    patient_id: str, observation_id: str
...
async def update_observation(
    patient_id: str, observation_id: str, observation_update_data: ObservationUpdate
) -> Optional[Observation]:
    ...
    if updated_snapshot.exists:
        return Observation(
            observation_id=updated_snapshot.id, **updated_snapshot.to_dict()
        )
    return None
...
async def create_medication_statement(
    patient_id: str, statement_data: MedicationStatementCreate
) -> MedicationStatement:
    ...
    subcollection_ref = (
        db.collection(NOAH_MVP_PATIENTS_COLLECTION)
        .document(patient_id)
        .collection(NOAH_MVP_MEDICATION_STATEMENTS_SUBCOLLECTION)
    )
    current_time = utcnow()
    ...
    _timestamp, doc_ref = await subcollection_ref.add(
        doc_data
    )  # Firestore auto-generates medication_statement_id
...
async def get_medication_statement(
    patient_id: str, statement_id: str
...
async def list_medication_statements_for_patient(
    ...
    async for snapshot in query.stream():
        if snapshot.exists:
            data = snapshot.to_dict()
            statements.append(
                MedicationStatement(medication_statement_id=snapshot.id, **data)
            )
    return statements


async def update_medication_statement(
    patient_id: str, statement_id: str, statement_update_data: MedicationStatementUpdate
) -> Optional[MedicationStatement]:
    ...
    if updated_snapshot.exists:
        return MedicationStatement(
            medication_statement_id=updated_snapshot.id, **updated_snapshot.to_dict()
        )
    return None
...
async def get_ai_contextual_store(patient_id: str) -> Optional[AIContextualStore]:
    ...
    if snapshot.exists:
        ...
        return AIContextualStore(
            **data
        )  # Assumes 'patient_id' is in data and matches snapshot.id
    return None


async def create_or_replace_ai_contextual_store(
    patient_id: str, store_data: AIContextualStoreCreate
) -> AIContextualStore:
    ...
    doc_data_dict[
        "patient_id"
    ] = patient_id  # Explicitly set patient_id from the path parameter
    ...
    await doc_ref.set(
        doc_data_dict
    )  # .set() overwrites the document or creates it if it doesn't exist.
...
async def update_ai_contextual_store(
    patient_id: str, store_update_data: AIContextualStoreUpdate
) -> Optional[AIContextualStore]:
    ...
    try:
        ...
        Exception
    ):  # google.cloud.exceptions.NotFound if doc doesn't exist with .update()
        ...
        await doc_ref.update(
            update_data_dict
        )  # Try update again if it was a race, or handle error
