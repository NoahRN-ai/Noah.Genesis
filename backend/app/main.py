...
from google.api_core import (
    exceptions as google_exceptions,  # For generic GCP error handling
)
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.error(
            f"Critical error during Firebase Admin SDK initialization: {e}",
            exc_info=True,
        )
    ...
    if firebase_admin._DEFAULT_APP_NAME in firebase_admin._apps:
        try:
            ...
            logger.error(
                f"Error deleting Firebase Admin app during shutdown: {e}", exc_info=True
            )
...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    ...
    logger.warning(
        f"Request validation error: {exc.errors()} for {request.method} {request.url.path}"
    )
    return JSONResponse(
...
async def http_exception_handler(request: Request, exc: HTTPException):
    ...
    logger.log(
        log_level,
        f"HTTPException: {exc.status_code} {exc.detail} for {request.method} {request.url.path}",
...
async def google_api_call_exception_handler(
    request: Request, exc: google_exceptions.GoogleAPICallError
):
    logger.error(
        f"Google API Call Error: {exc} for {request.method} {request.url.path}",
        exc_info=True,
    )
...
async def generic_exception_handler(request: Request, exc: Exception):
    ...
    logger.error(
        f"Unhandled exception: {exc} for {request.method} {request.url.path}",
        exc_info=True,
    )
    return JSONResponse(
