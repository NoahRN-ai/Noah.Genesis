        ...
        try:
            ...
            if settings.FIREBASE_SERVICE_ACCOUNT_JSON_PATH:
                cred = credentials.Certificate(
                    settings.FIREBASE_SERVICE_ACCOUNT_JSON_PATH
                )
            ...
            # Option 2: Rely on GOOGLE_APPLICATION_CREDENTIALS or default credentials in GCP environment
                ...
                logger.info(
                    "Firebase Admin SDK initialized using application default credentials or environment."
                )
...
http_bearer = HTTPBearer(
    auto_error=True
)  # auto_error will raise HTTPException for missing token
