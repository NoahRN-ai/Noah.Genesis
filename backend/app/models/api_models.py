...
from .firestore_models import InteractionActor, PatientDataLogDataType, UserRole
from .firestore_models import ToolCall as FirestoreToolCall
from .firestore_models import ToolResponse as FirestoreToolResponse


# --- Chat API Models ---
class ChatRequest(BaseModel):
    user_query_text: str
    user_voice_input_bytes: Optional[
        bytes
    ] = None  # Not used directly by LLM yet, for STT later
    session_id: Optional[
        str
    ] = None  # Client can manage session IDs, or server can generate
...
class InteractionOutput(BaseModel):  # API representation of an interaction
    ...
    message_content: str
    tool_calls: Optional[
        list[FirestoreToolCall]
    ] = None  # Re-using Firestore Pydantic model for structure
    tool_responses: Optional[
        list[FirestoreToolResponse]
    ] = None  # Re-using Firestore Pydantic model for structure
...
class PatientDataLogCreateInput(BaseModel):
    ...
    source: Optional[str] = Field(
        default="Noah.Genesis_MVP", examples=["Nurse Manual Entry", "Patient App Input"]
    )
...
class UserProfileUpdateInput(
    BaseModel
):  # Based on FirestoreUserProfileUpdate structure
