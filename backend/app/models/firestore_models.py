...
class ObservationBase(BaseModel):
    ...
    issued: Optional[datetime] = Field(default_factory=utcnow) # Date this version was released
    performer_user_id: Optional[List[str]] = Field(default_factory=list) # Who performed the observation (e.g., UserProfile.user_id)
    ...
    interpretation: Optional[List[CodeableConcept]] = Field(default_factory=list) # E.g., high, low, normal
...
class MedicationStatementBase(BaseModel):
    ...
    effectivePeriod_start: Optional[datetime] = None # More explicit than effectiveDateTime
    ...
    dateAsserted: Optional[datetime] = Field(default_factory=utcnow) # When this statement was recorded
    informationSource_user_id: Optional[str] = None # Who provided the information (e.g., patient, practitioner, UserProfile.user_id)
...
class MedicationStatement(MedicationStatementBase): # Represents a MedicationStatement document read from DB
...
class AIContextualStoreBase(BaseModel):
    ...
    preferences: Dict[str, Any] = Field(default_factory=dict) # E.g. communication style, summary length
    custom_alerts: List[str] = Field(default_factory=list) # Or List[Dict[str, Any]] for structured alerts
...
class AIContextualStore(AIContextualStoreBase): # Represents an AIContextualStore document read from DB
