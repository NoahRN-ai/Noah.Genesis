...
def transform_data_for_ai(raw_data: dict) -> dict:
    ...
    """
    # Using print for logging as in the original file, consider proper logging later
    print(f"[{datetime.datetime.utcnow().isoformat()}] INFO: Called transform_data_for_ai with raw_data:")
    # print(json.dumps(raw_data, indent=2)) # Commenting out to reduce noise for now

    transformed_data = {
        "processing_timestamp": datetime.datetime.utcnow().isoformat(),
        "original_data": raw_data,
        "ai_optimized_features": {
            "key_metric_1": raw_data.get("some_value", 0) * 2,
            "description": f"Transformed data for {raw_data.get('patientId', 'unknown_patient')}"
        }
    }

    # print(f"[{datetime.datetime.utcnow().isoformat()}] INFO: Transformation complete. Result:") # Commenting out
    # print(json.dumps(transformed_data, indent=2)) # Commenting out

    return transformed_data

if __name__ == "__main__":
    # This part is for example usage and might not be needed in the final utility file,
    # but keeping it for now as it was in the original.
    print("Running example usage of data_transformer.py (now in utils)...")
    ...
    print("Transformed Patient Data (example):", json.dumps(transformed_patient_data, indent=2))
    print("-" * 30)
    ...
    print("Transformed Event Data (example):", json.dumps(transformed_event_data, indent=2))
    print("-" * 30)
