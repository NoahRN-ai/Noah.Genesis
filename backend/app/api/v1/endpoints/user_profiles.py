...
from backend.app.models.firestore_models import UserProfileUpdate # For constructing service layer input
...
async def update_user_profile_endpoint( # Renamed to avoid conflict with service
    ...
    if not firestore_update_model.model_dump(exclude_unset=True): # Check if there's anything to update
    ...
    try:
        ...
        if not updated_profile_doc:
            ...
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User profile not found for update, or update failed.")
