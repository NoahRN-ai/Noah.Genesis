...
from backend.app.models.firestore_models import (
    PatientDataLogCreate,  # For constructing service layer input
)
...
@router.post(
    "", response_model=PatientDataLogResponse, status_code=status.HTTP_201_CREATED
)
async def submit_patient_data_log(
    data_input: PatientDataLogCreateInput,
    current_user: UserInfo = Depends(get_current_active_user),
    target_patient_user_id: str = Query(
        ..., description="The User ID of the patient this data log pertains to."
    ),
):
    ...
    try:
        ...
        logger.error(
            f"Error creating patient data log for user {target_patient_user_id} by {current_user.user_id}: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Could not create patient data log.",
        )


@router.get("", response_model=list[PatientDataLogResponse])
async def get_patient_data_logs(
    patient_user_id: str = Query(
        ..., description="User ID of the patient whose data logs are to be retrieved."
    ),
    current_user: UserInfo = Depends(get_current_active_user),
    limit: int = Query(default=20, ge=1, le=100),
    order_by: str = Query(
        default="timestamp",
        description="Field to order by, e.g., 'timestamp' or 'created_at'",
    ),
    ...
    if current_user.user_id != patient_user_id:
        ...
        logger.warning(
            f"User {current_user.user_id} attempt to access data logs for different user {patient_user_id}."
        )
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Not authorized to access this patient's data logs.",
        )
    ...
    try:
        ...
        logger.error(
            f"Error listing patient data logs for user {patient_user_id}: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Could not retrieve patient data logs.",
        )
