...
def run_patient_intake_workflow(patient_id: str):
    ...
    if profile_response.get("status") == "error":
        print(
            f"  [WORKFLOW_ERROR] Could not fetch patient profile: {profile_response.get('message')}"
        )
        print("--- Patient Intake Workflow Ended (with error) ---")
        return
    patient_profile = profile_response.get("data", {})
    print(
        f"  [WORKFLOW_STEP_1] Patient Profile fetched: {json.dumps(patient_profile, indent=2)}"
    )
    ...
    if events_response.get("status") == "error":
        print(
            f"  [WORKFLOW_WARN] Could not fetch patient events: {events_response.get('message')}. Proceeding with summary generation."
        )
        patient_events = []
    else:
        patient_events = events_response.get("data", [])
    print(
        f"  [WORKFLOW_STEP_2] Patient Events fetched: {len(patient_events)} events found."
    )
    if patient_events:
        print(
            f"    Last event timestamp (if any): {patient_events[0].get('timestamp') if patient_events else 'N/A'}"
        )  # Events are sorted desc by default by eldp_get_events
    ...
    if summary_response.get("status") == "error":
        print(
            f"  [WORKFLOW_ERROR] Could not generate patient summary: {summary_response.get('message')}"
        )
    else:
        intake_summary = summary_response.get("summary_text", "No summary generated.")
        print(
            f"  [WORKFLOW_STEP_3] Intake Summary generated (via Facade):\n{intake_summary}"
        )

    print("--- Patient Intake Workflow Completed ---")


def run_log_new_observation_workflow(
    patient_id: str, observation_text: str, source: str = "manual_text_entry"
):
    ...
    if log_response.get("status") == "error":
        print(
            f"  [WORKFLOW_ERROR] Failed to log observation: {log_response.get('message')}"
        )
    ...
    print(
        f"  [WORKFLOW_STEP_1] Observation logged via Facade. Event ID: {logged_event_id}"
    )
    ...
    events_response = agents.call_data_aggregation_alloydb_events(
        patient_id
    )  # Facade call
    patient_events_updated = events_response.get("data", [])

    print(
        f"  [WORKFLOW_STEP_2] Events fetched after logging: {len(patient_events_updated)} events."
    )
    # Check if the new event is present (optional check)
    found_new_event_in_refetch = any(
        evt.get("event_id") == logged_event_id for evt in patient_events_updated
    )
    print(
        f"    New event ({logged_event_id}) present in re-fetched events: {found_new_event_in_refetch}"
    )
    ...
    if patient_profile:
        ...
        if updated_summary_response.get("status") == "success":
            print(
                f"  [WORKFLOW_STEP_2] Updated Summary (via Facade):\n{updated_summary_response.get('summary_text')}"
            )
        else:
            print(
                f"  [WORKFLOW_WARN] Could not regenerate summary after logging event: {updated_summary_response.get('message')}"
            )
    else:
        print("  [WORKFLOW_WARN] Could not fetch profile to regenerate summary.")

    print("--- Log New Observation Workflow Completed ---")


def run_add_todo_task_workflow(
    patient_id_for_task: str, task_description: str, priority: str
):
    ...
    task_response = agents.call_todo_list_manager(
        action="add_task", payload=add_task_payload
    )

    if task_response.get("status") == "success":
        created_task = task_response.get("task", {})
        print(
            f"  [WORKFLOW_STEP_1] Task added successfully: {created_task.get('task_id')}"
        )
...
if __name__ == "__main__":
    ...
    run_log_new_observation_workflow(
        patient_id="patient002",
        observation_text="Patient reports feeling better after medication.",
    )

    # Workflow 3: Add To-Do Task
    run_add_todo_task_workflow(
        patient_id="patient001",
        task_description="Prepare discharge summary.",
        priority="High",
    )
    run_add_todo_task_workflow(
        patient_id="general_unit",
        task_description="Check emergency cart supplies.",
        priority="Medium",
    )
