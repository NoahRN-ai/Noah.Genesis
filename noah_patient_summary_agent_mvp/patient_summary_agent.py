...
MOCK_EVENT_LOGS = {
    "patient123": [
        {"eventId": "evt001", "patientId": "patient123", "eventType": "VitalSignChange", "timestamp": "2024-07-20T08:00:00Z", "value": {"temp_c": 39.1, "hr_bpm": 115, "bp_sys": 90, "bp_dia": 50, "spo2_pct": 91}, "source": "BedsideMonitor"},
        {"eventId": "evt002", "patientId": "patient123", "eventType": "MedicationAdministered", "timestamp": "2024-07-20T08:30:00Z", "value": {"medication": "BroadSpectrumAntibiotic IV", "dose": "2g"}, "source": "EMAR"},
        {"eventId": "evt003", "patientId": "patient123", "eventType": "LabResult", "timestamp": "2024-07-20T09:00:00Z", "value": {"test": "Lactate", "result": "4.5 mmol/L", "abnormal": True}, "source": "LIS"},
        {"eventId": "evt004", "patientId": "patient123", "eventType": "NurseInput", "timestamp": "2024-07-20T10:00:00Z", "value": {"note": "Patient verbalized feeling very weak. Family at bedside."}, "source": "ManualInput"}
    ],
    "patient456": [
        {"eventId": "evt005", "patientId": "patient456", "eventType": "VitalSignChange", "timestamp": "2024-07-20T09:00:00Z", "value": {"temp_c": 38.2, "hr_bpm": 98, "bp_sys": 130, "bp_dia": 75, "spo2_pct": 93, "rr_rpm": 22}, "source": "BedsideMonitor"},
        {"eventId": "evt006", "patientId": "patient456", "eventType": "MedicationAdministered", "timestamp": "2024-07-20T09:15:00Z", "value": {"medication": "Levofloxacin PO", "dose": "750mg"}, "source": "EMAR"},
        {"eventId": "evt007", "patientId": "patient456", "eventType": "NurseInput", "timestamp": "2024-07-20T11:00:00Z", "value": {"note": "Productive cough, sputum greenish. Encouraged deep breathing exercises."}, "source": "ManualInput"}
    ]
}

# Load mock clinical knowledge base
try:
    with open("mock_clinical_kb.json", 'r') as f:
...
def get_mock_patient_data(patient_id: str) -> dict:
    ...
    print(f"[{datetime.datetime.utcnow().isoformat()}] INFO: Fetching patient data for ID: {patient_id}")
...
def get_mock_event_logs(patient_id: str) -> list:
    ...
    print(f"[{datetime.datetime.utcnow().isoformat()}] INFO: Fetching event logs for ID: {patient_id}")
...
def query_vertex_ai_search(patient_data: dict, query: str) -> list:
    ...
    """
    print(f"[{datetime.datetime.utcnow().isoformat()}] INFO: Querying RAG (Vertex AI Search simulation) for patient {patient_data.get('patientId')} with query: '{query}'")

    retrieved_docs = []
    query_terms = query.lower().split()

    if not MOCK_CLINICAL_KB:
        print(f"[{datetime.datetime.utcnow().isoformat()}] WARN: Mock clinical knowledge base is empty.")
        return []
    ...
    print(f"[{datetime.datetime.utcnow().isoformat()}] INFO: RAG simulation retrieved {len(retrieved_docs)} documents.")
...
def generate_summary_with_llm(context_data: dict, summary_type: str) -> str:
    ...
    """
    print(f"[{datetime.datetime.utcnow().isoformat()}] INFO: Generating LLM summary (simulation). Type: {summary_type}")
    ...
    summary_parts.append(f"Patient: {patient_profile.get('name', 'N/A')} (ID: {patient_profile.get('patientId', 'N/A')})")
    summary_parts.append(f"DOB: {patient_profile.get('dob', 'N/A')}, MRN: {patient_profile.get('mrn', 'N/A')}")
    summary_parts.append(f"Admission Date: {patient_profile.get('admissionDate', 'N/A')}")
    summary_parts.append(f"Principal Problem: {patient_profile.get('principalProblem', 'N/A')}")
    summary_parts.append(f"Allergies: {', '.join(patient_profile.get('allergies', ['N/A']))}")
    ...
    if event_logs:
        for event in event_logs[:3]: # Show a few recent events
            summary_parts.append(f"  - {event.get('timestamp')}: {event.get('eventType')} - {json.dumps(event.get('value'))}")
    else:
    ...
    if rag_results:
        ...
        summary_parts.append("  - No specific clinical guidance retrieved by RAG simulation.")

    # Tailor based on summary_type
    if summary_type == 'nursing_note':
        summary_parts.append("\nNursing Focus:")
        summary_parts.append("  - Monitor vital signs closely, especially [relevant vital for principal problem].")
        ...
        summary_parts.append(f"  - Patient is {patient_profile.get('name')} admitted for {patient_profile.get('principalProblem')}.")
        summary_parts.append("  - Current status: [Simulated assessment - e.g., Stable but requires monitoring].")
        summary_parts.append("  - Pending tasks: [e.g., Follow up on morning labs, consult with specialist].")
        summary_parts.append("  - Key concerns: [e.g., Potential for deterioration, pain management].")
...
def get_patient_summary(patient_id):
    summary_type = request.args.get('type', 'handoff_report') # Default to handoff_report

    print(f"[{datetime.datetime.utcnow().isoformat()}] API: Received request for patient_id: {patient_id}, type: {summary_type}")
...
def run_direct_simulation(patient_id_to_test, summary_type_to_test):
    ...
    generated_summary_text = generate_summary_with_llm(context_data, summary_type_to_test)
...
if __name__ == "__main__":
    ...
    print("-" * 50)
    run_direct_simulation("patient_nonexistent", "handoff_report") # Test non-existent patient
