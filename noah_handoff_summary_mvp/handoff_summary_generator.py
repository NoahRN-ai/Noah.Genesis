...
from dateutil import (
    parser as dateutil_parser,  # For parsing ISO dates if needed for filtering
)
...
try:
    ...
    print(
        f"[{datetime.datetime.utcnow().isoformat()}] INFO: Successfully imported patient_summary_agent module."
    )
except ImportError as e:
    print(
        f"[{datetime.datetime.utcnow().isoformat()}] WARN: Could not import from patient_summary_agent.py: {e}. Using local stubs for profile & summary."
    )
...
try:
    ...
    events_log_dir = os.path.join(
        os.path.dirname(current_script_path), "noah_shift_events_log_display_mvp"
    )
    ...
    print(
        f"[{datetime.datetime.utcnow().isoformat()}] INFO: Successfully imported events_log_data_prep module."
    )
except ImportError as e:
    print(
        f"[{datetime.datetime.utcnow().isoformat()}] WARN: Could not import from events_log_data_prep.py: {e}. Using local stubs for events."
    )
    ...
    "patient789": [
        {
            ...
            "details": {"sign_name": "Pain Score", "value": "1/10", "unit": ""},
        },
        {
...
def get_patient_profile_stub(patient_id: str) -> dict:
    ...
    print(
        f"[{datetime.datetime.utcnow().isoformat()}] HANDOFF_FETCH_PROFILE: For patient_id: {patient_id}"
    )
    if IMPORTED_SUMMARY_AGENT_MODULE:
        ...
        if profile:
            return profile  # Return if found in imported module's data
...
def get_recent_events_stub(patient_id: str, shift_duration_hours: int = 12) -> list:
    ...
    print(
        f"[{datetime.datetime.utcnow().isoformat()}] HANDOFF_FETCH_EVENTS: For patient_id: {patient_id}, last {shift_duration_hours} hours."
    )
    if IMPORTED_EVENTS_LOG_MODULE:
        ...
        start_time_iso = (
            now - datetime.timedelta(hours=shift_duration_hours)
        ).isoformat()
        ...
        recent_events = [
            event
            for event in all_patient_events
...
def call_patient_summary_agent_for_handoff_stub(profile: dict, events: list) -> str:
    ...
    print(
        f"[{datetime.datetime.utcnow().isoformat()}] HANDOFF_AI_SUMMARY: Generating for {patient_name} using Patient Summary Agent logic."
    )
    ...
    if IMPORTED_SUMMARY_AGENT_MODULE:
        ...
        rag_results_for_handoff = PSA_MODULE.query_vertex_ai_search(
            profile, rag_query_for_handoff
        )
        ...
        summary_str = PSA_MODULE.generate_summary_with_llm(
            context_data, summary_type="handoff_concise"
        )
        return summary_str
    else:
        ...
        if events:
            ...
            for event in events[:3]:
                event_time = dateutil_parser.isoparse(event["timestamp"]).strftime(
                    "%H:%M"
                )
                summary_parts.append(
                    f"  - {event_time}: {event.get('event_type','N/A')} - {str(event.get('details','N/A'))[:50]}"
                )
        else:
...
def call_tts_service_stub(
    text_to_speak: str, language_code: str = "en-US", patient_id: str = "default"
) -> str:
    ...
    print(
        f'  Text to synthesize: "{text_to_speak[:100]}..." (length: {len(text_to_speak)})'
    )  # Print a snippet
...
def generate_handoff_data(
    patient_id: str,
    manual_top_priorities: list[str],
    manual_params_to_monitor: list[str],
) -> dict:
    ...
    if not profile:
        print(
            f"[{datetime.datetime.utcnow().isoformat()}] HANDOFF_GEN_ERROR: Profile not found for {patient_id}."
        )
    ...
    print(
        f"[{datetime.datetime.utcnow().isoformat()}] HANDOFF_GEN: Data generation complete for {patient_id}."
    )
    return handoff_data
...
if __name__ == "__main__":
    ...
    non_existent_patient_handoff = generate_handoff_data(
        "patient_unknown", ["Test priority"], ["Test param"]
    )
