...
def add_task(description: str, priority: str, patient_id: str = "general") -> dict:
    ...
    if priority not in PRIORITY_ORDER:
        print(f"[{datetime.datetime.utcnow().isoformat()}] WARN: Invalid priority '{priority}'. Defaulting to 'Medium'.")
        priority = "Medium"
    ...
    print(f"[{datetime.datetime.utcnow().isoformat()}] INFO: Task added: {task_id} - '{description}'")
    return new_task
...
def get_task(task_id: str) -> dict | None:
    ...
    for task in MOCK_TASKS_DB:
        if task["task_id"] == task_id:
            return task
    print(f"[{datetime.datetime.utcnow().isoformat()}] INFO: Task with ID {task_id} not found.")
    return None

def update_task_status(task_id: str, new_status: str) -> dict | None:
    """Updates the status of an existing task."""
    if new_status not in VALID_STATUSES:
        print(f"[{datetime.datetime.utcnow().isoformat()}] ERROR: Invalid status '{new_status}'. Must be one of {VALID_STATUSES}.")
        return None
    ...
    if task:
        ...
        print(f"[{datetime.datetime.utcnow().isoformat()}] INFO: Task {task_id} status updated to '{new_status}'.")
        return task
...
def get_all_tasks(patient_id_filter: str = None, sort_by_priority: bool = True) -> list:
    ...
    if patient_id_filter:
        ...
        tasks_to_return = [
            task for task in MOCK_TASKS_DB
    ...
    if sort_by_priority:
        ...
        tasks_to_return.sort(key=lambda t: (t["status"] != "Pending", -PRIORITY_ORDER.get(t["priority"], 0)))
    else:
        # Default sort by creation time if not by priority
        tasks_to_return.sort(key=lambda t: t["created_at"])

    print(f"[{datetime.datetime.utcnow().isoformat()}] INFO: Returning {len(tasks_to_return)} tasks.")
...
if __name__ == "__main__":
    ...
    # Add some tasks
    print("\n1. Adding tasks:")
    task1 = add_task("Administer morning medications for Patient A (Room 301)", "High", patient_id="PatientA_Room301")
    task2 = add_task("Follow up on pending lab results for Patient B (Room 302)", "Urgent", patient_id="PatientB_Room302")
    task3 = add_task("Restock isolation cart in Room 301", "Medium", patient_id="general_unit") # General unit task
    task4 = add_task("Complete shift handoff report documentation", "High", patient_id="general_reporting")
    task5 = add_task("Check IV site for Patient A (Room 301)", "Medium", patient_id="PatientA_Room301")
    task_low = add_task("Review new unit protocols (non-urgent)", "Low", patient_id="general_unit")
    ...
    for task in all_tasks_sorted:
        print(f"  - [{task['status']}] {task['description']} (Priority: {task['priority']}, Patient: {task.get('patient_id', 'N/A')})")
    ...
    for task in patient_a_tasks:
        print(f"  - [{task['status']}] {task['description']} (Priority: {task['priority']})")
    ...
    if task2:
        ...
        if updated_task2:
            print(f"  Task '{updated_task2['description']}' is now '{updated_task2['status']}'.")
    ...
    for task in all_tasks_after_update:
        print(f"  - [{task['status']}] {task['description']} (Priority: {task['priority']}, Patient: {task.get('patient_id')})")

    print("\n7. Get a specific task (task1):")
    specific_task = get_task(task1["task_id"])
    if specific_task:
        print(f"  Found task: {specific_task['description']}, Status: {specific_task['status']}")

    print("\n8. Try to update with invalid status:")
    update_task_status(task1["task_id"], "InProgress") # This is an invalid status as per current VALID_STATUSES
