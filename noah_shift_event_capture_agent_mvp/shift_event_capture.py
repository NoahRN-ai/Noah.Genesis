...
def load_event_schemas(schema_file_path="event_schemas.json"):
    ...
    try:
        ...
        print(
            f"[{datetime.datetime.utcnow().isoformat()}] INFO: Event schemas loaded successfully from {schema_file_path}"
        )
    except FileNotFoundError:
        print(
            f"[{datetime.datetime.utcnow().isoformat()}] ERROR: Schema file {schema_file_path} not found. No schema validation will be performed."
        )
        EVENT_SCHEMAS = {}
    except json.JSONDecodeError:
        print(
            f"[{datetime.datetime.utcnow().isoformat()}] ERROR: Error decoding JSON from {schema_file_path}. No schema validation will be performed."
        )
        EVENT_SCHEMAS = {}
...
def validate_event_details(event_type: str, event_details: dict) -> bool:
    ...
    if not schema:
        print(
            f"[{datetime.datetime.utcnow().isoformat()}] WARN: No schema found for event type '{event_type}'. Skipping validation."
        )
    ...
    if missing_keys:
        print(
            f"[{datetime.datetime.utcnow().isoformat()}] ERROR: Event validation failed for type '{event_type}'. Missing keys: {missing_keys}. Event: {event_details}"
        )
        return False
...
def store_event_in_alloydb(event_data: dict):
    ...
    if patient_id:
        ...
        print(
            f"[{datetime.datetime.utcnow().isoformat()}] SIMULATE_DB_WRITE (SHARED_MOCK_EVENTS_DB): Event {shared_event_record['event_id']} for patient {patient_id} logged."
        )
        print(
            f"  Current SHARED_MOCK_EVENTS_DB for {patient_id}: {SHARED_MOCK_EVENTS_DB[patient_id][-1]}"
        )  # Print last added event
    else:
        ...
        if (
            shared_event_record.get("event_type") == "alert_notification"
            and not patient_id
        ):
            ...
            print(
                f"[{datetime.datetime.utcnow().isoformat()}] SIMULATE_DB_WRITE (SHARED_MOCK_EVENTS_DB): System Alert {shared_event_record['event_id']} logged under '{system_alerts_key}'."
            )
        else:
            print(
                f"[{datetime.datetime.utcnow().isoformat()}] WARN_DB_WRITE: Event {shared_event_record['event_id']} lacks patient_id and is not a general system alert. Not added to SHARED_MOCK_EVENTS_DB by default."
            )
...
def transcribe_voice_input(mock_audio_blob: str) -> str:
    ...
    """
    print(
        f"[{datetime.datetime.utcnow().isoformat()}] SIMULATE_STT: Transcribing mock audio blob: {mock_audio_blob}"
    )
...
def log_event(
    ...
    """
    if validate and EVENT_SCHEMAS.get(
        event_type
    ):  # Only validate if schema exists for this type
        if not validate_event_details(event_type, event_details):
            print(
                f"[{datetime.datetime.utcnow().isoformat()}] INFO: Event logging aborted due to validation failure for event type '{event_type}'."
            )
            return
    ...
    print(
        f"[{datetime.datetime.utcnow().isoformat()}] LOGGING_EVENT: Type: {event_type}, Patient: {patient_id}"
    )
...
def log_vital_sign(patient_id: str, sign_name: str, value: any, unit: str):
    ...
    return log_event(
        patient_id, "vital_sign", details, source_override="ClinicalDevice_ManualEntry"
    )


def log_intervention(
    patient_id: str, description: str, medication: dict = None, procedure: str = None
):
    ...
    return log_event(
        patient_id, "intervention", details, source_override="NurseInput_Intervention"
    )


def log_observation(
    patient_id: str, text_observation: str, source: str = "manual_text"
):
...
def log_general_note(
    patient_id: str, note_content: str, source: str = "manual_text_note"
):
...
def log_general_note_from_voice(patient_id: str, mock_audio_blob: str):
    ...
    print(
        f"[{datetime.datetime.utcnow().isoformat()}] INFO: Initiating voice note for patient {patient_id} using {mock_audio_blob}"
    )
    transcribed_text = transcribe_voice_input(mock_audio_blob)
    if transcribed_text:
        return log_general_note(
            patient_id, transcribed_text, source="voice_transcription_note"
        )
    else:
        print(
            f"[{datetime.datetime.utcnow().isoformat()}] ERROR: Transcription failed or returned empty for {mock_audio_blob}. Note not logged."
        )
        return None
...
def log_alert_notification(
    ...
    priority: str,
    patient_id: str = None,
    source_system: str = "ApplicationInternal",
):
    ...
    return log_event(
        ...
        "alert_notification",
        details,
        source_override=f"AlertSystem_{source_system}",
    )
...
if __name__ == "__main__":
    ...
    print("---")
    log_vital_sign(
        patient_id_example, sign_name="Blood Pressure", value="120/80", unit="mmHg"
    ...
    log_general_note_from_voice(
        patient_id_example, mock_audio_blob="patient_reports_pain_in_knee.wav"
    )
    print("---")
    log_general_note_from_voice(
        patient_id_example_2,
        mock_audio_blob="doctor_ordered_new_medication_for_fever.wav",
    )
    print("---")
    ...
    log_alert_notification(
        message="Low disk space on server XYZ.",
    ...
    log_alert_notification(
        patient_id=patient_id_example_2,
    ...
    log_event(
        patient_id_example,
        "intervention",
        {"procedure_performed": "Test procedure only"},
    )
    print("---")
