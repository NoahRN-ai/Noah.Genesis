...
from dateutil import (
    parser as dateutil_parser,  # Using dateutil.parser for robust ISO date parsing
)
...
try:
    ...
    sibling_script_dir = os.path.join(
        os.path.dirname(current_script_path), "noah_shift_event_capture_agent_mvp"
    )
    ...
    print(
        f"[{datetime.datetime.utcnow().isoformat()}] INFO: Successfully imported SHARED_MOCK_EVENTS_DB from shift_event_capture.py"
    )

except ImportError as e:
    print(
        f"[{datetime.datetime.utcnow().isoformat()}] WARN: Could not import SHARED_MOCK_EVENTS_DB from shift_event_capture.py: {e}"
    )
    print(
        f"[{datetime.datetime.utcnow().isoformat()}] WARN: Using a local mock store for events_log_data_prep.py. Data will not be shared from event capture for this run."
    )
    IMPORTED_SHARED_MOCK_EVENTS_DB = {  # Local fallback for demonstration if import fails
        "patientX789": [
            {
                "event_id": "local_evt001",
                "timestamp": "2024-07-21T08:00:00Z",
                "patient_id": "patientX789",
                "event_type": "vital_sign",
                "source": "BedsideMonitor",
                "details": {"sign_name": "Heart Rate", "value": 75, "unit": "bpm"},
            },
            {
                "event_id": "local_evt003",
                "timestamp": "2024-07-21T08:05:00Z",
                "patient_id": "patientX789",
                "event_type": "observation",
                "source": "manual_text",
                "details": {"text_observation": "Patient resting (local data)."},
            },
        ]
    }
except AttributeError as e:
    print(
        f"[{datetime.datetime.utcnow().isoformat()}] WARN: SHARED_MOCK_EVENTS_DB not found in imported shift_event_capture module: {e}"
    )
    print(
        f"[{datetime.datetime.utcnow().isoformat()}] WARN: Using a local mock store. Ensure SHARED_MOCK_EVENTS_DB is defined in shift_event_capture.py."
    )
    IMPORTED_SHARED_MOCK_EVENTS_DB = {  # Local fallback
        "patientX789": [
            {
                "event_id": "attr_err_evt001",
                "timestamp": "2024-07-21T08:00:00Z",
                "patient_id": "patientX789",
                "event_type": "vital_sign",
                "source": "BedsideMonitor",
                "details": {"sign_name": "Heart Rate", "value": 70, "unit": "bpm"},
            }
        ]
    }
...
def get_mock_shift_events(patient_id: str, filter_criteria: dict = None) -> list:
    ...
    """
    print(
        f"[{datetime.datetime.utcnow().isoformat()}] DB_ACCESS: Getting shift events for {patient_id} from SHARED_MOCK_EVENTS_DB with filters: {filter_criteria}"
    )
    ...
    if not patient_events_list:
        ...
        if patient_id == "_system_alerts" or (
            not filter_criteria and not patient_events_list
        ):
            patient_events_list = list(
                IMPORTED_SHARED_MOCK_EVENTS_DB.get("_system_alerts", [])
            )
            if patient_events_list:
                print(
                    f"[{datetime.datetime.utcnow().isoformat()}] DB_ACCESS: Found {len(patient_events_list)} system alerts."
                )

    if not patient_events_list:
        print(
            f"[{datetime.datetime.utcnow().isoformat()}] DB_ACCESS: No events found for {patient_id}."
        )
        return []
    ...
    if filter_criteria:
        ...
        if time_range:
            ...
            start_time = (
                dateutil_parser.isoparse(start_time_str) if start_time_str else None
            )
            ...
            if start_time:
                filtered_events = [
                    ...
                    if dateutil_parser.isoparse(
                        event.get("timestamp", "1900-01-01T00:00:00Z")
                    )
                    >= start_time
                ]
            if end_time:
                filtered_events = [
                    ...
                    for event in filtered_events
                    if dateutil_parser.isoparse(
                        event.get("timestamp", "9999-12-31T23:59:59Z")
                    )
                    <= end_time
                ]
    ...
    print(
        f"[{datetime.datetime.utcnow().isoformat()}] DB_ACCESS: Returning {len(filtered_events)} events for {patient_id} after filtering."
    )
...
if __name__ == "__main__":
    ...
    print("--- Testing Event Log (potentially from SHARED_MOCK_EVENTS_DB) ---")
    ...
    if all_events_shared:
        ...
        print(
            f"  No events found for {test_patient_id_shared} in SHARED_MOCK_EVENTS_DB."
        )
        print(
            "  Note: Run shift_event_capture.py's main block in the same session or ensure"
        )
        print(
            "  SHARED_MOCK_EVENTS_DB is populated via other means for this to show shared data."
        )
    ...
    vital_sign_events = get_mock_shift_events(
        test_patient_id_shared, filter_criteria=vital_signs_filter
    )
    if vital_sign_events:
        ...
        print(
            f"  No vital sign events found for {test_patient_id_shared} with this filter."
        )
    ...
    time_filter_start = {
        "time_range": {"start": "2024-07-21T09:00:00Z"}
    }  # Use a relevant time for your test data
    if IMPORTED_SHARED_MOCK_EVENTS_DB.get(test_patient_id_shared):  # If there's data
        ...
        first_event_ts = IMPORTED_SHARED_MOCK_EVENTS_DB[test_patient_id_shared][0][
            "timestamp"
        ]
        # time_filter_start = {"time_range": {"start": first_event_ts}} # Example of dynamic filter

    late_events = get_mock_shift_events(
        test_patient_id_shared, filter_criteria=time_filter_start
    )
    if late_events:
