...
try:
    ...
    summary_agent_dir = os.path.join(
        os.path.dirname(current_script_path), "noah_patient_summary_agent_mvp"
    )

    if summary_agent_dir not in sys.path:
        sys.path.insert(0, summary_agent_dir)

    from patient_summary_agent import MOCK_CLINICAL_KB as PSA_MOCK_CLINICAL_KB
    from patient_summary_agent import MOCK_EVENT_LOGS as PSA_MOCK_EVENT_LOGS
    ...
    from patient_summary_agent import get_mock_patient_data as psa_get_mock_patient_data
    from patient_summary_agent import (
    ...
    print(
        f"[{datetime.datetime.utcnow().isoformat()}] INFO: Successfully imported functions from patient_summary_agent.py."
    )
except ImportError as e:
    print(
        f"[{datetime.datetime.utcnow().isoformat()}] WARN: Could not import from patient_summary_agent.py: {e}."
    )
    print(
        f"[{datetime.datetime.utcnow().isoformat()}] WARN: report_data_generator.py will use its own local mocks for patient profiles and RAG summary."
    )
    # Define local fallbacks if import fails
    PSA_MOCK_PATIENT_PROFILES = {
        "patientA123": {
            "patientId": "patientA123",
            "name": "Eleanor Vance (Local Fallback)",
            "dob": "1958-03-12",
        ...
        "patientB456": {
            ...
            "name": "Marcus Cole (Local Fallback)",
            "dob": "1972-11-25",
...
def get_mock_patient_profile(patient_id: str) -> dict:
    ...
    """
    print(
        f"[{datetime.datetime.utcnow().isoformat()}] MOCK_DB_FETCH (Report Gen): Getting patient profile for {patient_id}"
    )
    if IMPORTED_PATIENT_SUMMARY_AGENT_FUNCTIONS:
        # Fetch core profile from patient_summary_agent's data
        base_profile = psa_get_mock_patient_data(
            patient_id
        )  # This is actually direct dict access in psa
        if (
            not base_profile
        ):  # psa_get_mock_patient_data in summary agent returns None if not found
            base_profile = PSA_MOCK_PATIENT_PROFILES.get(
                patient_id, {}
...
def get_mock_firestore_data(
    patient_id: str, data_key: str
) -> dict:  # Renamed from original to avoid conflict
    """Simulates fetching specific data (e.g., 'About Me') for a patient from Firestore."""
    print(
        f"[{datetime.datetime.utcnow().isoformat()}] MOCK_DB_FETCH (Report Gen): Getting Firestore data for {patient_id}, key: {data_key}"
    )
...
def fetch_patient_history_summary_from_rag(patient_id: str) -> dict:
    ...
    """
    print(
        f"[{datetime.datetime.utcnow().isoformat()}] RAG_CALL (Report Gen): Fetching history summary for {patient_id} via Patient Summary Agent logic."
    )

    if not IMPORTED_PATIENT_SUMMARY_AGENT_FUNCTIONS:
        print(
            f"[{datetime.datetime.utcnow().isoformat()}] RAG_CALL_WARN: Patient Summary Agent functions not imported. Using local RAG fallback."
        )
        ...
        original_mock_rag = {  # Keep a copy of the original simple mock for this fallback
            "patientA123": {
                "past_medical_history": "Hypertension (10 years), Type 2 Diabetes (5 years).",
                "surgical_history": "Appendectomy (1985).",
            },
            "patientB456": {
                "past_medical_history": "Asthma (childhood).",
                "surgical_history": "None.",
            },
        }
        return original_mock_rag.get(
            patient_id,
            {
    ...
    if not patient_data_for_rag:
        return {
            "past_medical_history": "Patient profile not found for RAG.",
            "surgical_history": "",
        }
    ...
    rag_results = psa_query_vertex_ai_search(
        patient_data_for_rag, rag_query
    )  # Uses PSA_MOCK_CLINICAL_KB
    ...
    llm_summary_response = psa_generate_summary_with_llm(
        context_for_llm, summary_type="history_recap"
    )
    ...
    if rag_results:
        for doc in rag_results:
            ...
            if (
                "protocol" in doc.get("title", "").lower()
                or "management" in doc.get("title", "").lower()
            ):
...
def populate_section_i_system_overview() -> dict:
    ...
    print(
        f"[{datetime.datetime.utcnow().isoformat()}] REPORT_GEN: Populating Section I - System Overview"
    )
...
def populate_section_ii_patient_identification(patient_id: str) -> dict:
    ...
    print(
        f"[{datetime.datetime.utcnow().isoformat()}] REPORT_GEN: Populating Section II - Patient ID for {patient_id}"
    )
    ...
    return {
        ...
        "consulting_physicians": ", ".join(
            profile.get("consulting_physicians", ["None listed"])
        ),
...
def populate_section_iii_patient_history(patient_id: str) -> dict:
    ...
    print(
        f"[{datetime.datetime.utcnow().isoformat()}] REPORT_GEN: Populating Section III - Patient History for {patient_id}"
    )
    ...
    social_history_data = get_mock_firestore_data(
        patient_id, "social_history_highlights"
    )
...
def save_to_firestore(patient_id: str, data_key: str, content: dict):
    ...
    """
    print(
        f"[{datetime.datetime.utcnow().isoformat()}] MOCK_DB_SAVE (Firestore): Saving data for patient {patient_id}"
    )
    ...
    if patient_id in MOCK_FIRESTORE_DATA:
        MOCK_FIRESTORE_DATA[patient_id][data_key] = content.get(
            data_key, content
        )  # Adapt based on content structure
    else:
        MOCK_FIRESTORE_DATA[patient_id] = {data_key: content.get(data_key, content)}
    print(
        f"[{datetime.datetime.utcnow().isoformat()}] MOCK_DB_SAVE: Data for {patient_id} - {data_key} (simulated) saved/updated."
    )
...
if __name__ == "__main__":
    ...
    section_ii_data_nonexistent = populate_section_ii_patient_identification(
        mock_patient_id_nonexistent
    )
    ...
    "patientA123": [
        {
            "site": "Left Forearm",
            "type": "PIV 20G",
            "patency": "Flushes well",
            "s_s_infection": "None noted",
            "dressing_change_due": "2024-07-22",
        },
        {
            "site": "Right Internal Jugular",
            "type": "Triple Lumen CVC",
            "patency": "All ports patent",
            "s_s_infection": "Slight redness at insertion",
            "dressing_change_due": "2024-07-25",
        },
    ],
    "patientB456": [
        {
            ...
            "type": "PIV 22G",
            "patency": "Sluggish return",
            "s_s_infection": "None noted",
            "dressing_change_due": "2024-07-21",
        }
    ],
}
...
def get_mock_lab_results(patient_id: str) -> dict:
    print(
        f"[{datetime.datetime.utcnow().isoformat()}] MOCK_DATA_FETCH: Getting lab results for {patient_id}"
    )
    return MOCK_LAB_RESULTS.get(
        patient_id, {"abnormal_results": [], "pending_results": []}
    )


def get_mock_iv_access_details(patient_id: str) -> list:
    print(
        f"[{datetime.datetime.utcnow().isoformat()}] MOCK_DATA_FETCH: Getting IV access details for {patient_id}"
    )
...
def populate_neuro_assessment(patient_id: str) -> dict:
    ...
    if patient_id == "patientA123":
        assessment[
            "notes"
        ] = "Patient reports slight dizziness on standing. Neuro checks Q4H."
    return assessment
...
def populate_cardiovascular_assessment(patient_id: str) -> dict:
    ...
    if patient_id == "patientA123":
        assessment[
            "heart_rhythm_rate"
        ] = "Atrial Fibrillation, Rate 88 bpm (controlled)"
        assessment["bp_map"] = "110/70 mmHg (MAP 83 mmHg)"
        assessment["edema"] = "+1 pitting edema bilateral ankles and shins."
        assessment[
            "active_vasoactive_drips"
        ] = "Furosemide 20mg IV BID"  # Not vasoactive, but relevant med.
    return assessment
...
def populate_gu_assessment(patient_id: str) -> dict:
    ...
    if patient_id == "patientA123":
        profile = get_mock_patient_profile(
            patient_id
        )  # To get admission date for context
        assessment[
            "renal_labs_summary"
        ] = f"BUN 25 mg/dL, Creatinine 1.8 mg/dL (Elevated, baseline {profile.get('principalProblem')})"  # Mock
...
def populate_skin_mobility_assessment(patient_id: str) -> dict:
    ...
    if patient_id == "patientA123":
        ...
        ] = "Dry skin noted on lower legs. Heels intact. Small ecchymosis on left forearm."
        assessment[
            "pressure_injury_prevention"
        ] = "Float heels, Mepilex to sacrum, turn Q2H."
        assessment[
            "mobility_status"
        ] = "Ambulates with rolling walker, SBA due to fatigue."
    return assessment
...
def populate_iv_access_assessment(patient_id: str) -> dict:
    ...
    return {
        ...
        "central_line_necessity": "Not applicable"
        if not any("CVC" in site.get("type", "") for site in iv_sites)
        else "Daily review for necessity, indication: medication admin",
        "notes": "Monitor right IJ CVC site for increased redness."
        if patient_id == "patientA123"
        else "Ensure PIVs are dated.",
    }
...
def populate_fluids_infusions_assessment(patient_id: str) -> dict:
    ...
    if patient_id == "patientA123":
        assessment[
            "ivf_maintenance_bolus"
        ] = "0.9% Normal Saline @ 30 mL/hr KVO via CVC."
        assessment[
            "fluid_balance_24hr"
        ] = "-200 mL (I:1500 O:1700) - Diuresing as planned."
    return assessment
...
def populate_pain_assessment(patient_id: str) -> dict:
    ...
    if patient_id == "patientB456":  # Pneumonia patient might have pleuritic pain
        ...
        ] = "Scheduled Acetaminophen 650mg Q6H. Last dose 06:00."
    return assessment
...
def populate_laboratory_diagnostics_assessment(patient_id: str) -> dict:
    ...
    return {
        "abnormal_lab_test_results": labs.get(
            "abnormal_results", []
        ),  # List of strings
        "pending_labs_tests": labs.get("pending_results", []),  # List of strings
        "recent_imaging_highlights": "Chest X-Ray (yesterday): Bilateral lower lobe infiltrates consistent with pneumonia."
        if patient_id == "patientB456"
        else "Chest X-Ray (today): Mild pulmonary congestion, improved from admission.",
        "other_diagnostics_notes": "ECG (today): Atrial Fibrillation, rate 88. No acute ST changes."
        if patient_id == "patientA123"
        else "Awaiting sputum culture results before antibiotic adjustment.",
    }
...
def populate_section_v_systems_assessment(patient_id: str) -> dict:
    ...
    print(
        f"[{datetime.datetime.utcnow().isoformat()}] REPORT_GEN: Populating Section V - Systems Assessment for {patient_id}"
    )

    if not get_mock_patient_profile(patient_id):  # Check if patient exists
        return {
            "error": f"Patient ID {patient_id} not found, cannot generate Section V."
        }
    ...
    return {
        ...
        "laboratory_diagnostics": populate_laboratory_diagnostics_assessment(
            patient_id
        ),
    }
